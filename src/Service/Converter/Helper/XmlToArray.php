...
class XmlToArray
{
     ...
     * @throws Exception
     */
    public static function &createArray($inputXml, $config = array()): array
    {
     ...
     * @throws Exception
     */
    public function &buildArray($inputXml): array
    {
        $this->init();

        if (is_string($inputXml)) {
            $this->xml = $this->createDOMDocument();
            $parsed = $this->xml->loadXML($inputXml,  LIBXML_PARSEHUGE);
            if ( ! $parsed) {
     ...
     * @param DOMNode $node
     */
    protected function collateNamespaces(DOMNode $node): void
    {
